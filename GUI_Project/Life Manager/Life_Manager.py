...
earns the user one pomodoro tomato graphics to the bottom of the screen. Total time
worked is counted together daily and displayed for motivation. (for evaluator:
add more time to cumulative timer in user_data.txt to see more tomatos. Format is
date; cumulative_timer; hours:mins:secs)
...
and squats.
    Minimal GUI button displays the work timer as a small window which stays
always in front of all other windows so can easily see how much time is left.
    Daily averages are calculated from the log file for work timer and all
excercises. Update button recalculates them after a change.
...
# Main graphical window of the app.
class GUI:
    ...
    def minimal_GUI(self):
        """
        Creates a small window with the work timer on the lower right area
        of the screen. Will always display on top of all other windows.
        """
    ...
    def minimal_GUI_timer(self):
        """
        Updates the minimal GUI timer once per second.
        """
        if self.__timer.is_running():
            print_timer = datetime(year=2021,
                                   month=12,
                                   day=25,
                                   hour=0,
                                   minute=0,
                                   second=0) + self.__timer.get_running_time()
    ...
    def break_timer_length(self):
        """
        Shows which break timer length radio button is selected.
        5 or 15 minutes.
        """
    ...
    def update_earned_pomodoros(self):
        """
        Updates the tomato graphics. Shows 5 tomatos per column.
        After 24 tomatos displayed just shows the number of extras.
        """
        self.__earned_pomodoros = self.__cumulative_time // timedelta(minutes=25)
        self.__pomodoro_label = []
    ...
    def update_averages(self):
        """
        Recalculates the averages for timer and excercises.
        """
    ...
    def daily_average(self, event):
        """
        Calculates all the daily averages from the logs.
        """
        previous_day = None
        daily_sum = 0
        daily_sums_list = []
    ...
    def get_todays_cumulative(self):
        """
        Get todays cumulative work timer.
        """
    ...
    def write_data(self, data_type, data, time=0):
        """
        Updates the log in User_data.txt file.
        """
        if time == 0:
            time = datetime.now()
    ...
    def clock(self):
        """
        Updates current clock time every second.
        """
    ...
    def current_timer(self):
        """
        Updates the current work timer and rings alarm after 25 minutes.
        """
        if self.__timer.is_running():
    ...
    def current_break_timer(self):
        """
        Updates break timer. Counts in reverse.
        Uses Colors green for break time and red for overtime.
        """
    ...
    def stop_timer(self):
        """
        Stops the timer and records the log.
        Needs to take into account midnight and changing of the day.
        """
        self.__play_alarm = False
        if self.__timer.is_running():
    ...
    def todays_working_time(self):
        """
        Updates the cumulative work timer for today.
        """
        if self.__timer.is_running():
...
class Timer:
    """
    Class to run the different kinds of timers.
    """
...
def select_user():
    """
    Calls the class i created for selecting a user. Creates a new GUI.
...
def read_data(username):
    """
    Reads the user data. Creates new log files if they're missing.
    :param username: name of the user
    :return: data_list, list of all the user's logged data
    """
    try:
